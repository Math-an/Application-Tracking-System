...
function JobList() {
  ...
  const handleHire = (jobId) => {
    axios.post('http://localhost:3001/hire', { id: jobId })
      .then(response => {
  ...
  const handleReject = (jobId) => {
    axios.delete(`http://localhost:3001/reject/${jobId}`)
      .then(response => {
        console.log(response.data.message);
        ...
        {jobs.map((job, index) => (
          <div key={index} className="m-3">
            ...
            <div className="mt-5 h-10 w-full bg-white flex">
              ...
              <button onClick={() => handleHire(job.id)} className="bg-green-600 h-6 w-16 text-white text-sm text-center relative mt-2 left-44 rounded">Hire</button>
              <button onClick={() => handleReject(job.id)} className="bg-red-500 h-6 w-16 text-white text-sm text-center mt-2 relative left-48 rounded">Reject</button>
